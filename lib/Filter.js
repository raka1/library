    ...
    template = `
      ...
      <script>
        ...
        function isNumber(evt, field) {
          ...
          if (charCode === 13) direct(evt, field);
          if (charCode > 31 && (charCode < 48 || charCode > 57))
            return false;
          return true;
        }
        function direct(field, value = null, evt) {
          console.log(evt);
          let urlQuery = '${urlQuery}';
          let newValues = [];
          urlQuery = urlQuery.split('&');
      ...
      <div class='filterRange'>
        ...
        <div>
          <input class='${f}' type='text' ` +
              `onkeypress='return isNumber(event, "${f}")' placeholder='${p[0]}' ` +
              `onfocusout='direct("${f}", null, event)'>
        </div>
        <div>
          <input class='${f}' type='text' ` + 
              `onkeypress='return isNumber(event, "${f}")' placeholder='${p[1]}' ` +
              `onfocusout='direct("${f}")'>
        </div>
      </div>
    `;
