        ...
        function load() {
          ...
          for (var i = 0; i < check.length; i++) {
            ...
            for (var j = 1; j < checkLength; j++) {
              console.log(urlQuery, checks[j].children[0].className);
            }
          }
        }
  ...
  filterRange(template, i) {
    ...
    template = `
      ...
      <div class='filterRange'>
        ...
        <div>
          <input class='${f}' type='text' placeholder='${p[0]}' ` +
              `onfocusout='direct("${f}")'>
        </div>
        <div>
          <input class='${f}' type='text' placeholder='${p[1]}' ` + 
              `onfocusout='direct("${f}")'>
        </div>
      </div>
    `;
