          ...
          if (target.from.hasOwnProperty('table') &&
            ...
            if (typeof target.from.table === 'string') {
              if (target.from.hasOwnProperty('on')) {
                from = `${target.from.table} JOIN ${target.from.join} ON '${Object.keys(target.from.on)[0]}' = '${Object.values(target.from.on)[0]}'`;
              } else if (target.from.hasOwnProperty('using')) {
                from = `${target.from.table} JOIN ${target.from.join} USING(${target.from.using})`;
              } else {
