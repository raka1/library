  ...
  render(callback) {
    ...
    if (this.thisUrl.query.action === 'add') {
      ...
      form.parse(this.request, (error, fields, files) => {
        ...
        this.#fs.renameSync(this.#path.normalize(files[this.bookField].path),
            this.#path.normalize(__dirname + '/../../../' + this.dir + '/' +
      ...
      <script>
        ...
        function add() {
          document.getElementById('crud').innerHTML = \`
            <form action="${this.thisUrl.pathname}?action=add" method='post' enctype="multipart/form-data">
              ${Object.keys(this.form)
                  .map((index) => `
                    <div>
                    <input name='${this.form[index]}' type="text"placeholder='${this.form[index]}' />
                    </div>
                  `).join('')}
              <div><input name='${this.bookField}' type="file" required /></div>
              <div>
          ...
          document.getElementById('crud').innerHTML = \`
            <form action="${this.thisUrl.pathname}?action=add" method='post'>
              ${Object.keys(this.form)
                  .map((index) => `
                    <div>
                    <input name='${this.form[index]}' type="text" placeholder='${this.form[index]}' value='' />
                    </div>
                  `).join('')}
              <div>
                <input name='_token' type="text" value='xF5Ls12lqp' hidden='' />
