...
class Search {
  ...
  renderMultiSearch(callback) {
    if (typeof this.action === 'undefined') {
      callback('Render multi search error: setAction(args) is undefined.', null);
    }
    if (typeof this.field === 'undefined') {
      callback('Render multi search error: setField(args) is undefined.', null);
    }
    if (typeof this.operator === 'undefined') {
      callback('Render multi search error: setOperator(args) is undefined.', null);
    }
    let submit;
    let operator = '';
  ...
  renderSearch(callback) {
    ...
    if (typeof this.attribute === 'undefined') {
      callback('Render multi search error: setAttribute(args) is undefined.', null);
    }
    let placeholder;
    let submit;
    if (typeof this.hook.placeholder !== 'undefined') {
