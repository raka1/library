class DocServeController {
  ...
  listTemplate(data, rows, rowsAll) {
    ...
    if (typeof thisUrl.query._page === 'undefined' || thisUrl.query._page === '') thisUrl.query._page = 1;
    template = `
      ${template}
      <style>
        .pagination {
    ...
    for (i = 0; i < page; i++) {
      if (thisUrl.query._page === i+1) {
        ...
        template = `${template}<span><a href="${queryParams}&_page=${i+1}">${i+1}</a></span>`;
      }
    }
        ...
        if (data.hasOwnProperty('idField') &&
          ...
          if (data.hasOwnProperty('request')) {
            ...
            this.query(`SELECT ${data.idField}, ${data.titleField}, ${data.descField}, ` +
                `${data.yearField} FROM ${data.table}${filter} LIMIT ${(thisUrl.query._page-1)*data.maxPerPage}, ${data.maxPerPage}`)
                .then(rows => {
                  this.query(`SELECT ${data.idField}, ${data.titleField}, ${data.descField}, ` +
                      `${data.yearField} FROM ${data.table}${filter}`)
                      .then(rowsAll => resolve(this.listTemplate(data, rows, rowsAll)))
                      .catch(err => console.log(err));
                })
                .catch(err => console.log(err));
          } else {
            this.query(`SELECT ${data.idField}, ${data.titleField}, ${data.descField}, ` +
                `${data.yearField} FROM ${data.table} LIMIT ${(thisUrl.query._page-1)*data.maxPerPage}, ${data.maxPerPage}`)
                .then(rows => {
                  this.query(`SELECT ${data.idField}, ${data.titleField}, ${data.descField}, ` +
