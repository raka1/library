    ...
    return new Promise((resolve, reject) => {
      if (!Array.isArray(data) && typeof data === 'object') {
        if (data.hasOwnProperty('idField') &&
            ...
            data.hasOwnProperty('dirImageField') &&
            data.hasOwnProperty('dirLink') &&
            data.hasOwnProperty('table')) {
          if (data.hasOwnProperty('request')) {
            let query;
            ...
            if (data.request.method === 'GET') {
              ...
              this.query(`SELECT ${data.idField}, ${data.titleField}, ${data.imageField} ` +
                  `FROM ${data.table}${filter}`)
                  .then(rows => resolve(this.tileTemplate(data, rows)))
                  .catch(err => console.log(err));
            } else {
              let body = '';
              data.request.on('data', chuck => {
