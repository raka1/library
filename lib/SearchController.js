        ...
        if (request.hasOwnProperty('action') &&
          ...
          if (request.hasOwnProperty('operator')) {
            for (var i = 0; i < request.field.length-1; i++) {
              if (request.operator[i] === undefined)
                request.operator[i] = 'or';
          ...
          resolve(`
            <form action='${request.action}' method='get'${id}>
              ${Object.keys(request.field).map((index) => {
                return `
                  <input name='${request.field[index]}' type='text' ` +
                      `placeholder='${request.cssPlaceholder[index]}'/>
                `
              }).join('')}
          ...
          resolve(`
            <form action='${request.action}' method='get'${id}>
              ...
              <input name='_attribute' type='text' ` +
                  `value='${request.attribute}' hidden autocomplete="off" />
              <input type='submit'${submit} />
            </form>
          `);
        } else {
