    ...
    return new Promise((resolve, reject) => {
      ...
      if (request.method == 'GET') {
        ...
        for (let [property, value] of Object.entries(thisUrl.query)) {
          ...
          if (typeof request.model.searchBy[property] === 'undefined') {
            request.model.searchBy[property] = property;
          }
          where = `${where}${request.model.searchBy[property]} LIKE \'%${value}%\'`;
          if (i === Object.keys(thisUrl.query).length) {
            break;
          } else {
            where = `${where} AND `;
